import { ValueEventType } from "@prisma/client";
import { prisma } from "./prisma";

export type ValueEventDTO = {
  id: string;
  label: string;
  type: ValueEventType;
  amountCents: number;
  currency: string;
  occurredAt: string;
  isEstimated: boolean;
  sourceId?: string | null;
};

export type AtRiskItem = {
  id: string;
  label: string;
  kind: "RENEWAL" | "RETURN_DEADLINE" | "REFUND_OVERDUE";
  dueDate: string;
  amountCents?: number | null;
  currency?: string | null;
  meta?: {
    status?: string | null;
    autopay?: boolean;
    store?: string | null;
  };
};

export type ValueSummary = {
  saved: {
    totalCents: number;
    confirmedCents: number;
    estimatedCents: number;
    currency: string;
    events: ValueEventDTO[];
  };
  atRisk: {
    totalCents: number;
    currency: string;
    horizonDays: number;
    renewals: AtRiskItem[];
    returnDeadlines: AtRiskItem[];
    overdueRefunds: AtRiskItem[];
  };
  recovered: {
    totalCents: number;
    currency: string;
    events: ValueEventDTO[];
  };
  plan: {
    code: "FREE" | "PRO";
    featureAccess: {
      saved: boolean;
      recovered: boolean;
      drilldowns: boolean;
      concierge: boolean;
    };
  };
};

function addDays(base: Date, days: number) {
  const d = new Date(base);
  d.setUTCDate(d.getUTCDate() + days);
  return d;
}

function sumCents(values: Array<number | null | undefined>): number {
  return values.reduce((sum: number, v) => sum + (typeof v === "number" ? v : 0), 0);
}

type RateMap = Record<string, number>;

function rateKey(from: string, to: string) {
  return `${from.toUpperCase()}->${to.toUpperCase()}`;
}

function convertAmountCents(amountCents: number | null | undefined, from: string | null | undefined, to: string, rates: RateMap) {
  if (amountCents == null) return null;
  const src = (from ?? to).toUpperCase();
  const dest = to.toUpperCase();
  if (src === dest) return amountCents;
  const direct = rates[rateKey(src, dest)];
  if (typeof direct === "number" && Number.isFinite(direct) && direct > 0) {
    return Math.round(amountCents * direct);
  }
  const inverse = rates[rateKey(dest, src)];
  if (typeof inverse === "number" && Number.isFinite(inverse) && inverse > 0) {
    return Math.round(amountCents / inverse);
  }
  return amountCents; // fallback: leave unconverted if no rate available
}

async function buildRateMap(displayCurrency: string, currencies: string[], asOf: Date): Promise<RateMap> {
  const upperDisplay = displayCurrency.toUpperCase();
  const unique = Array.from(new Set(currencies.map(c => c.toUpperCase()).filter(Boolean)));
  if (unique.length === 0) return {};

  // Fetch latest rates where base is EUR (ECB-style) for all needed quotes, including display
  const rows = await prisma.fxRate.findMany({
    where: {
      baseCurrency: "EUR",
      quoteCurrency: { in: Array.from(new Set([...unique, upperDisplay])) },
      asOfDate: { lte: asOf },
    },
    orderBy: [{ quoteCurrency: "asc" }, { asOfDate: "desc" }],
  });

  const euroTo: Record<string, number> = {};
  for (const row of rows) {
    if (euroTo[row.quoteCurrency]) continue; // keep latest per quote
    euroTo[row.quoteCurrency] = Number(row.rate);
  }

  const map: RateMap = {};
  const eurToDisplay = upperDisplay === "EUR" ? 1 : euroTo[upperDisplay];

  for (const cur of unique) {
    if (cur === upperDisplay) {
      map[rateKey(cur, upperDisplay)] = 1;
      continue;
    }
    if (cur === "EUR") {
      if (eurToDisplay) {
        map[rateKey("EUR", upperDisplay)] = eurToDisplay;
        map[rateKey(upperDisplay, "EUR")] = 1 / eurToDisplay;
      }
      continue;
    }
    const eurToCur = euroTo[cur];
    if (eurToCur && eurToDisplay) {
      const cross = eurToDisplay / eurToCur; // cur -> display
      map[rateKey(cur, upperDisplay)] = cross;
      map[rateKey(upperDisplay, cur)] = 1 / cross;
    }
  }

  return map;
}

export async function computeValueSummary(
  userId: string,
  opts?: { horizonDays?: number; displayCurrency?: string }
): Promise<ValueSummary> {
  const horizonDays = opts?.horizonDays ?? 7;
  const billing = await prisma.billingAccount.findUnique({ where: { userId }, select: { plan: true, preferredCurrency: true } });
  const displayCurrency = (opts?.displayCurrency ?? billing?.preferredCurrency ?? "CAD").toUpperCase();
  const now = new Date();
  const horizonDate = addDays(now, horizonDays);

  const [planRow, subscriptions, returns, valueEvents] = await Promise.all([
    Promise.resolve(billing),
    prisma.subscription.findMany({
      where: { userId, status: "ACTIVE" },
      select: { id: true, name: true, amountCents: true, currency: true, renewalDate: true, status: true },
    }),
    prisma.returnItem.findMany({
      where: { userId },
      select: {
        id: true,
        store: true,
        itemNote: true,
        amountCents: true,
        refundAmountCents: true,
        currency: true,
        returnBy: true,
        refundExpectedAt: true,
        deliveredAt: true,
        refundSlaDays: true,
        status: true,
        refundedDate: true,
      },
    }),
    prisma.valueEvent.findMany({
      where: { userId },
      orderBy: { occurredAt: "desc" },
      take: 100,
    }),
  ]);

  const currencies = [
    displayCurrency,
    ...subscriptions.map(s => s.currency),
    ...returns.map(r => r.currency),
    ...valueEvents.map(ev => ev.currency),
  ].filter(Boolean) as string[];

  const rates = await buildRateMap(displayCurrency, currencies, now);

  const planCode = planRow?.plan ?? "FREE";
  const featureAccess = {
    saved: planCode !== "FREE",
    recovered: planCode !== "FREE",
    drilldowns: planCode !== "FREE",
    concierge: false,
  };

  const savedEvents: ValueEventDTO[] = valueEvents
    .filter(ev => ev.type !== ValueEventType.REFUND_RECEIVED)
    .map(ev => ({
      id: ev.id,
      label: ev.sourceId ? `${ev.type.replace("_", " ")}` : ev.type,
      type: ev.type,
      amountCents: convertAmountCents(ev.amountCents, ev.currency, displayCurrency, rates) ?? 0,
      currency: displayCurrency,
      occurredAt: ev.occurredAt.toISOString(),
      isEstimated: ev.isEstimated,
      sourceId: ev.sourceId,
    }));

  const savedConfirmed = sumCents(savedEvents.filter(ev => !ev.isEstimated).map(ev => ev.amountCents));
  const savedEstimated = sumCents(savedEvents.filter(ev => ev.isEstimated).map(ev => ev.amountCents));

  const recoveredMap = new Map<string, ValueEventDTO>();
  valueEvents
    .filter(ev => ev.type === ValueEventType.REFUND_RECEIVED)
    .forEach(ev => {
      const key = ev.sourceId ? `ve-${ev.sourceId}` : ev.id;
      recoveredMap.set(key, {
        id: ev.id,
        label: "Refund received",
        type: ev.type,
        amountCents: convertAmountCents(ev.amountCents, ev.currency, displayCurrency, rates) ?? 0,
        currency: displayCurrency,
        occurredAt: ev.occurredAt.toISOString(),
        isEstimated: ev.isEstimated,
        sourceId: ev.sourceId,
      });
    });

  returns
    .filter(r => r.refundedDate)
    .forEach(r => {
      const key = `return-${r.id}`;
      if (recoveredMap.has(key)) return;
      recoveredMap.set(key, {
        id: key,
        label: r.store,
        type: ValueEventType.REFUND_RECEIVED,
        amountCents: convertAmountCents(r.refundAmountCents ?? r.amountCents ?? 0, r.currency, displayCurrency, rates) ?? 0,
        currency: displayCurrency,
        occurredAt: r.refundedDate!.toISOString(),
        isEstimated: false,
        sourceId: r.id,
      });
    });

  const recoveredEvents = Array.from(recoveredMap.values()).sort((a, b) => b.occurredAt.localeCompare(a.occurredAt));

  const renewalRisks: AtRiskItem[] = subscriptions
    .filter(s => s.renewalDate >= now && s.renewalDate <= horizonDate)
    .map(s => ({
      id: s.id,
      label: s.name,
      kind: "RENEWAL" as const,
      dueDate: s.renewalDate.toISOString(),
      amountCents: convertAmountCents(s.amountCents, s.currency, displayCurrency, rates),
      currency: displayCurrency,
      meta: { status: s.status },
    }));

  const returnDeadlineRisks: AtRiskItem[] = returns
    .filter(r => r.status !== "REFUNDED" && r.returnBy >= now && r.returnBy <= horizonDate)
    .map(r => ({
      id: r.id,
      label: r.itemNote ? `${r.store} — ${r.itemNote}` : r.store,
      kind: "RETURN_DEADLINE" as const,
      dueDate: r.returnBy.toISOString(),
      amountCents: convertAmountCents(r.amountCents ?? r.refundAmountCents ?? null, r.currency, displayCurrency, rates),
      currency: displayCurrency,
      meta: { status: r.status, store: r.store },
    }));

  const overdueRefunds: AtRiskItem[] = returns
    .filter(r => r.status !== "REFUNDED")
    .map(r => {
      const expected = r.refundExpectedAt ?? (r.deliveredAt ? addDays(r.deliveredAt, r.refundSlaDays ?? 14) : null);
      return { record: r, expected } as const;
    })
    .filter(({ expected }) => Boolean(expected && expected < now))
    .map(({ record, expected }) => ({
      id: `${record.id}-overdue`,
      label: record.itemNote ? `${record.store} — refund` : `${record.store} refund`,
      kind: "REFUND_OVERDUE" as const,
      dueDate: expected!.toISOString(),
      amountCents: convertAmountCents(record.refundAmountCents ?? record.amountCents ?? null, record.currency, displayCurrency, rates),
      currency: displayCurrency,
      meta: { status: record.status, store: record.store },
    }));

  const atRiskTotal = sumCents([
    ...renewalRisks.map(r => r.amountCents ?? 0),
    ...returnDeadlineRisks.map(r => r.amountCents ?? 0),
    ...overdueRefunds.map(r => r.amountCents ?? 0),
  ]);

  const summary: ValueSummary = {
    saved: {
      totalCents: savedConfirmed + savedEstimated,
      confirmedCents: savedConfirmed,
      estimatedCents: savedEstimated,
      currency: displayCurrency,
      events: savedEvents.slice(0, 25),
    },
    atRisk: {
      totalCents: atRiskTotal,
      currency: displayCurrency,
      horizonDays,
      renewals: renewalRisks.slice(0, 25),
      returnDeadlines: returnDeadlineRisks.slice(0, 25),
      overdueRefunds: overdueRefunds.slice(0, 25),
    },
    recovered: {
      totalCents: sumCents(recoveredEvents.map(ev => ev.amountCents)),
      currency: displayCurrency,
      events: recoveredEvents.slice(0, 25),
    },
    plan: {
      code: planCode,
      featureAccess,
    },
  };

  return summary;
}
